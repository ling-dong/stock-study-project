...
def save_spas_history(
    etf_code: str, etf_name: str, created_at: str,
    probability: float, direction: str, position_pct: float,
    stop_loss: float, take_profit: float,
    psychology_score: int, psychology_level: str,
    inputs_json: str, result_json: str,
) -> int:
    conn = _get_conn()
    cursor = conn.execute("""
        INSERT INTO spas_history
        (etf_code, etf_name, created_at, probability, direction,
         position_pct, stop_loss, take_profit,
         psychology_score, psychology_level, inputs_json, result_json)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        etf_code, etf_name, created_at, probability, direction,
        position_pct, stop_loss, take_profit,
...
def get_spas_history(limit: int = 20) -> list[dict]:
    conn = _get_conn()
    rows = conn.execute(
        "SELECT * FROM spas_history ORDER BY id DESC LIMIT ?", (limit,)
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def get_spas_history_entry(entry_id: int) -> Optional[dict]:
    conn = _get_conn()
    row = conn.execute(
        "SELECT * FROM spas_history WHERE id = ?", (entry_id,)
    ).fetchone()
    conn.close()
    return dict(row) if row else None


def delete_spas_history(entry_id: int) -> bool:
    conn = _get_conn()
    cursor = conn.execute("DELETE FROM spas_history WHERE id = ?", (entry_id,))
    conn.commit()
    deleted = cursor.rowcount > 0
    conn.close()
    return deleted
