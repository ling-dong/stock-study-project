...
with st.sidebar:
    ...
    # ── 统计数据 ──
    try:
        from core.db.repository import get_all_chapter_progress
...
def _render_home():
    ...
    # 继续学习
    try:
        from core.db.repository import get_user_preferences
...
def _render_chapter_list(phase_id: str):
    from core.engine.content_loader import load_chapter
    ...
    try:
        from core.db.repository import get_chapter_progress
    except Exception:
...
def _render_reading_view(phase_id: str, chapter_id: str):
    from core.engine.content_loader import load_chapter, load_quiz
    from streamlit_app.widgets.quiz_widget import render_quiz
    from core.db.repository import save_chapter_progress, get_chapter_progress
...
def _render_sandbox():
    ...
    try:
        from streamlit_app.pages.sandbox import show as show_sandbox
        show_sandbox()
    except ImportError:
        st.info("🚧 交易沙盒正在建设中…")
    except Exception as e:
...
def _render_progress():
    ...
    try:
        from core.db.repository import get_all_chapter_progress, get_quiz_results
        all_p = get_all_chapter_progress()
    except Exception as e:
